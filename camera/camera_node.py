...
def callback(msg):
    ...
    #image_path=rospack.get_path('camera') + '/images/'
    #msg.data= cv2.imread(image_path +'sizeDetect.jpg')
    #image_mesage=bridge.imgmsg_to_cv2(msg.data,"passthrough")
    nr_rows = msg.height
    nr_columns = msg.width
    pas=msg.step
    codare=msg.encoding
    #image_mesage=bridge.imgmsg_to_cv2(msg.data, "32FC1")
    ...
    #print(image_mesage[500][900])
        
    for i in range(nr_rows):
     for j in range(nr_columns):
       elemente = [msg.data[i*nr_rows+4*j],msg.data[i*nr_rows+4*j+1],msg.data[i*nr_rows+4*j+2],msg.data[i*nr_rows+4*j+3]]
    ...
    print("mijloc")
    for j in range(nr_columns):
       distanta[539][j]=My_mat[539][j]   #linia de la jumate va fi exact aia
    ...
    ls.angle_min=-0.52
    ls.angle_max=0.52
    ls.angle_increment=0.0005
    ls.header.frame_id="base_scan"
    ls.time_increment=0	
    ls.scan_time=0
    ls.range_min=0.1199
    ls.range_max=4
    ls.ranges=np.copy(distanta[539,:])
    #ls.intensities
    pub.publish(ls)
